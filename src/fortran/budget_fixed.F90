...
contains
   ...
   if (nliving .lt. 100) call OMP_SET_NUM_THREADS(1)
      ...
      evavg = sum(evap(1:nliving) * ocp_coeffs(1:nliving), mask = .not. ieee_is_nan(evap(1:nliving)))
      phavg = sum(ph(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(ph(1:nliving)))
      aravg = sum(ar(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(ar(1:nliving)))
      nppavg = sum(nppa(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(nppa(1:nliving)))
      laiavg = sum(laia(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(laia(1:nliving)))
      rcavg = sum(rc2(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(rc2(1:nliving)))
      f5avg = sum(f5(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(f5(1:nliving)))
      rmavg = sum(rm(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(rm(1:nliving)))
      rgavg = sum(rg(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(rg(1:nliving)))
      wueavg = sum(wue(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(wue(1:nliving)))
      cueavg = sum(cue(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(cue(1:nliving)))
      c_defavg = sum(c_def(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(c_def(1:nliving))) / 2.73791
      vcmax_1 = sum(vcmax(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(vcmax(1:nliving)))
      specific_la_1 = sum(specific_la(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(specific_la(1:nliving)))
      litter_l_1 = sum(litter_l(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(litter_l(1:nliving)))
      cwd_1 = sum(cwd(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(cwd(1:nliving)))
      litter_fr_1 = sum(litter_fr(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(litter_fr(1:nliving)))
      c_cost_cwm = sum(npp2pay(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(npp2pay(1:nliving)))

      cp(1) = sum(cl1_int(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(cl1_int(1:nliving)))
      cp(2) = sum(ca1_int(1:nliving) * (ocp_coeffs(1:nliving) * idx_grasses(1:nliving)), mask= .not. ieee_is_nan(ca1_int(1:nliving)))
      cp(3) = sum(cf1_int(1:nliving) * ocp_coeffs(1:nliving), mask= .not. ieee_is_nan(cf1_int(1:nliving)))
      cp(4) = sum(ar_fix_hr(1:nliving) * (ocp_coeffs(1:nliving) * idx_pdia(1:nliving)), mask= .not. ieee_is_nan(ar_fix_hr(1:nliving)))

      do p = 1,2
         nupt_1(p) = sum(nupt(p,1:nliving) * ocp_coeffs(1:nliving))
      enddo
      do p = 1,3
         pupt_1(p) = sum(pupt(p,1:nliving) * ocp_coeffs(1:nliving))
      enddo

      do p = 1, 6
         lit_nut_content_1(p) = sum(lit_nut_content(p, 1:nliving) * ocp_coeffs(1:nliving))
      enddo
